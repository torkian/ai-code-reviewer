...
class TestOpenAIClient:
    ...
    def test_analyze_code_with_ai_success(self, mock_call_api, sample_diff):
        ...
        assert (
            ...
            "Good code changes with proper structure"
        )
    ...
    @patch.dict('os.environ', {'OPENAI_API_KEY': 'sk-test-key'})
    @patch('src.utils.openai_client.call_openai_api')
    def test_analyze_code_with_ai_invalid_json(
            self, mock_call_api, sample_diff
    ):
        ...
        assert (
            "Analysis completed, but the result format was unexpected" in
            result['overall_comment']
        )
        assert result['file_comments'] == []
    ...
    @patch.dict('os.environ', {'OPENAI_API_KEY': 'sk-test-key'})
    @patch('src.utils.openai_client.call_openai_api')
    def test_analyze_code_with_ai_json_in_markdown(
            self, mock_call_api, sample_diff
    ):
    ...
    @patch.dict('os.environ', {'OPENAI_API_KEY': 'sk-test-key'})
    @patch('src.utils.openai_client.call_openai_api')
    def test_analyze_code_with_ai_error_response(
            self, mock_call_api, sample_diff
    ):
