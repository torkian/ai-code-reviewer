...
def get_pr_diff(pr_info):
    ...
    try:
        ...
        if 'links' in pr_info and 'diff' in pr_info['links']:
            ...
            diff_url = f"{BITBUCKET_API_BASE}/repositories/{pr_info['repository']['full_name']}/pullrequests/{pr_info['id']}/diff"
        ...
        logger.info(f"Successfully retrieved diff ({len(diff_content)} characters)")
        
        # Truncate very large diffs to avoid OpenAI token limits
        max_diff_size = 20000
        if len(diff_content) > max_diff_size:
            logger.warning(f"Diff is very large ({len(diff_content)} chars), truncating to {max_diff_size} chars")
            diff_content = diff_content[:max_diff_size] + f"\n... [Diff truncated, total size: {len(diff_content)} chars]"
...
def post_comment_to_pr(pr_info, comment):
    ...
    try:
        url = f"{BITBUCKET_API_BASE}/repositories/{pr_info['repository']['full_name']}/pullrequests/{pr_info['id']}/comments"
...
def normalize_path(file_path):
    ...
    if not normalized or normalized.startswith('..'):
        logger.warning(f"Invalid path format: {file_path}, normalized to: {normalized}")
...
def extract_files_from_diff(diff_content):
    ...
    matches = re.findall(r'diff --git a/(.*?) b/(.*?)$', diff_content, re.MULTILINE)
    ...
    try:
        ...
        if not isinstance(line_number, int):
            try:
                ...
                logger.error(f"Invalid line number: {line_number} - must be an integer")
                return False
        ...
        logger.info(f"Normalized path from '{file_path}' to '{normalized_path}'")
        ...
        if response.status_code == 200 or response.status_code == 201:
            logger.info(f"Successfully posted inline comment to PR {pr_info['id']} at {normalized_path}:{line_number}")
            return True
        else:
            # Log the error for debugging
            error_detail = response.json() if response.text else "No error details"
            logger.error(f"Failed to post inline comment: {response.status_code} - {error_detail}")
            return False
...
def get_latest_commit_id(pr_info):
    ...
    try:
        url = f"{BITBUCKET_API_BASE}/repositories/{pr_info['repository']['full_name']}/pullrequests/{pr_info['id']}/commits"
