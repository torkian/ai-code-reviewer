...
def call_openai_api(prompt, system_message=None):
    ...
    try:
        ...
        if response.status_code != 200:
            error_info = response.json() if response.text else "No error details available"
            logger.error(f"OpenAI API error: {response.status_code} - {error_info}")
            return f"Error: The AI service returned an error (HTTP {response.status_code}). Please check the logs for details."
        ...
        return "Error: The AI service request timed out. Please try again later."
    except json.JSONDecodeError as e:
        logger.error(f"JSON decode error from OpenAI response: {str(e)}")
        return "Error: Could not parse the AI service response. Please check the logs."
...
"""
    ...
    try:
        ...
        return {"overall_comment": f"Failed to analyze code due to an error: {str(e)}"}