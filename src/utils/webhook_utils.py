...
def verify_webhook_signature(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not WEBHOOK_SECRET:
            logger.warning(
                "Webhook secret not configured - skipping signature "
                "verification"
            )
...
def extract_pr_info(payload):
    ...
    return {
        ...
        "source_branch": (
            pullrequest.get("source", {}).get("branch", {}).get("name")
        ),
        "destination_branch": (
            pullrequest.get("destination", {}).get("branch", {}).get("name")
        ),
        "repository": {
            "full_name": (
                pullrequest.get("destination", {})
                .get("repository", {})
                .get("full_name")
            ),
        },
