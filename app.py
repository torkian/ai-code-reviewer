...
LOG_FILE = os.path.join(
    LOG_DIR,
    f"ai_reviewer_{datetime.datetime.now().strftime('%Y%m%d')}.log"
)
    ...
    def decorated_function(*args, **kwargs):
        ...
        if count > API_RATE_LIMIT:
            logger.warning(
                ...
                f"{count} requests in the current hour"
            )
            return jsonify({
                "status": "error",
                "message": (
                    f"Rate limit exceeded. Maximum {API_RATE_LIMIT} "
                    "requests per hour."
                )
            }), 429
...
def webhook():
    ...
    for comment in analysis.get("file_comments", []):
        if ("file" in comment and "line_number" in comment and
                "comment" in comment):
            ...
            if ("```" in comment_text and
                    not re.search(r"```\w+", comment_text)):
            ...
            # If inline comment fails, add it to the list for a
            # fallback comment
            if not result:
                logger.warning(
                    "Failed to post inline comment, adding to fallback list"
                )
    ...
    for doc in analysis.get("documentation", []):
        if "file" in doc and "line_number" in doc and "doc_comment" in doc:
            ...
            if not result:
                logger.warning(
                    "Failed to post documentation suggestion, "
                    "adding to fallback list"
                )
                failed_comments.append({
                    "file": file_path,
                    "line": line_number,
    ...
    if failed_comments:
        logger.info(
            f"Posting fallback comment with {len(failed_comments)} "
            "failed inline comments"
        )
        fallback_text = (
            "## Inline Comments (Could not post directly)\n\n"
        )

        # Add all comments without categories
        for comment in failed_comments:
            fallback_text += (
                f"**{comment['file']} (line {comment['line']}):**\n"
                f"{comment['comment']}\n\n---\n\n"
            )
